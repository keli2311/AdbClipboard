...
class WriteReceiverTest : BaseTest() {
    ...
    fun writeClipboard_success() {
        ...
        // The clipboard is now written asynchronously by the bridge activity
        waitForClipboardValue(testText)
    }
    ...
    private fun waitForClipboardValue(expected: String) {
        val deadline = System.currentTimeMillis() + 5000
        while (System.currentTimeMillis() < deadline) {
            if (directClipboardRead() == expected) return
            Thread.sleep(100)
        }
        assertEquals("wrong value in clipboard", expected, directClipboardRead())
    }
}
