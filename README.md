...
🖱️ **Paste on Android** - Tap the floating paste button to paste into the focused app  
...
**Paste on Android (Manual)**: The floating window also has a paste button. Tapping it asks the PC
script to send a paste keyevent over ADB, so the content currently on your Android clipboard is
pasted into the app that is focused behind the floating window.
...
6. To paste into an app on your phone, focus its text field and tap the paste button on the
   floating window (the PC script must be running)
...
```bash
# Request a paste (this is what the floating paste button does)
adb shell touch /sdcard/Android/data/ch.pete.adbclipboard/files/paste_request.txt

# The Python script picks up the request and pastes the device clipboard
# into the focused field using:
