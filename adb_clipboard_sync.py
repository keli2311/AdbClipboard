...
class CommandRunner:
    ...
    def run_command(self, cmd: List[str], timeout: int = DEFAULT_TIMEOUT, 
                   input_text: Optional[str] = None, context: str = "command",
                   encoding: str = "utf-8") -> Optional[subprocess.CompletedProcess]:
        """
        ...
        Args:
            ...
            encoding: Encoding used to decode command output (UTF-8 by default,
                      which matches the text written by the Android app)
    ...
    def _is_expected_failure(self, context: str, result: subprocess.CompletedProcess) -> bool:
        ...
        # Checking for a paste request when the file doesn't exist yet is expected
        if result.stderr and "No such file or directory" in result.stderr:
            return "check paste request" in context
        return False
...
class AdbManager:
    ...
    def __init__(self, command_runner: CommandRunner, logger: logging.Logger, config: Config):
        ...
        self.logger = logger
        self.config = config
        # Allow bundling/overriding the adb binary (used by the packaged PC app)
    ...
    def paste_from_device(self, device_hash: str) -> bool:
        """Paste the device clipboard into the focused field if a paste was requested"""
        result = self.run_adb_command([
            '-s', device_hash,
            'shell', 'ls', PASTE_REQUEST_FILE_PATH
        ], context=f"check paste request on {device_hash}")

        if result is None or result.returncode != 0:
            return False

        # Clear the request first so a single tap only triggers one paste
        self.run_adb_command([
            '-s', device_hash,
            'shell', 'rm', PASTE_REQUEST_FILE_PATH
        ], timeout=10, context=f"clear paste request on {device_hash}")

        result = self.run_adb_command([
            '-s', device_hash,
            'shell', 'input', 'keyevent', KEYCODE_PASTE
        ], context=f"send paste to {device_hash}")

        if result is not None and result.returncode == 0:
            self.logger.debug(f"Sent paste keyevent to {device_hash}")
            return True

        self.logger.warning(f"Failed to send paste keyevent to {device_hash}")
        return False
...
class WindowsClipboardHandler(ClipboardHandler):
    ...
    def read_clipboard(self) -> str:
        ...
        # Route through a UTF-8 temp file instead of parsing console output,
        # so non-ASCII text works regardless of the console code page
        tmp_path = os.path.join(tempfile.gettempdir(), 'adb_clipboard_read.txt')
        result = self.runner.run_command([
            'powershell', '-NoProfile', '-Command',
            f'Get-Clipboard | Set-Content -LiteralPath "{tmp_path}" -Encoding UTF8'
    ...
    def write_clipboard(self, text: str) -> None:
        ...
        tmp_path = os.path.join(tempfile.gettempdir(), 'adb_clipboard_write.txt')
        try:
...
class ClipboardSyncManager:
    ...
    def sync_with_devices(self) -> None:
        ...
        while True:
            try:
                ...
                if self._handle_paste_requests(devices):
                    continue
    ...
    def _sync_clipboard_to_devices(self, devices: List[str]) -> bool:
        ...
        if self.previous_clipboard != current_clipboard and current_clipboard:
            ...
            elif self.on_clipboard_synced:
                self.on_clipboard_synced(current_clipboard)
            
            return True
        
        return False

    def _handle_paste_requests(self, devices: List[str]) -> bool:
        """Paste on devices whose floating paste button was tapped"""
        paste_requested = False

        for device in devices:
            if self.adb_manager.paste_from_device(device):
                self.logger.info(f"Paste triggered on {device}")
