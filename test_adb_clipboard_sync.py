...
class TestCommandRunner(unittest.TestCase):
    ...
    @patch('subprocess.run')
    def test_successful_command_custom_encoding(self, mock_run):
        ...
        result = self.runner.run_command(
            ['echo', 'test'], encoding='gbk', context="test command"
        )
    ...
    def test_expected_failure_not_logged(self, mock_run):
        ...
        mock_result.returncode = 1
        mock_result.stdout = ""
        mock_result.stderr = "ls: /path/paste_request.txt: No such file or directory"
        mock_run.return_value = mock_result
        
        # When
        result = self.runner.run_command(
            ['adb', '-s', 'test123', 'shell', 'ls', '/path/paste_request.txt'],
            context="check paste request on test123"
        )
...
class TestAdbManager(unittest.TestCase):
    ...
    def test_paste_from_device_no_request(self):
        """Test paste when no request file exists"""
        # Given
        mock_result = Mock()
        mock_result.returncode = 1
        mock_result.stdout = ""
        mock_result.stderr = f"ls: {PASTE_REQUEST_FILE_PATH}: No such file or directory"
        self.runner_mock.run_command.return_value = mock_result

        # When
        result = self.adb_manager.paste_from_device("device123")

        # Then
        self.assertFalse(result)
        self.assertEqual(self.runner_mock.run_command.call_count, 1)

    def test_paste_from_device_success(self):
        """Test successful paste request handling"""
        # Given
        self.runner_mock.run_command.side_effect = [
            Mock(returncode=0, stdout=f"{PASTE_REQUEST_FILE_PATH}\n", stderr=""),
        ...
        # Then
        self.assertTrue(result)
        calls = [call.args[0] for call in self.runner_mock.run_command.call_args_list]
        self.assertEqual(calls[0], [
            'adb', '-s', 'device123', 'shell', 'ls', PASTE_REQUEST_FILE_PATH
        ])
        self.assertEqual(calls[1], [
            'adb', '-s', 'device123', 'shell', 'rm', PASTE_REQUEST_FILE_PATH
        ])
        self.assertEqual(calls[2], [
            'adb', '-s', 'device123', 'shell', 'input', 'keyevent', '279'
        ])

    def test_paste_from_device_keyevent_failure(self):
        """Test paste when the keyevent command fails"""
        # Given
        self.runner_mock.run_command.side_effect = [
            Mock(returncode=0, stdout=f"{PASTE_REQUEST_FILE_PATH}\n", stderr=""),
            Mock(returncode=0, stdout="", stderr=""),
        ...
        # Then
        self.assertFalse(result)
        self.assertEqual(self.runner_mock.run_command.call_count, 3)
        self.logger.warning.assert_called_with("Failed to send paste keyevent to device123")
...
class TestClipboardHandlers(unittest.TestCase):
    ...
    @patch('adb_clipboard_sync.tempfile.gettempdir', return_value='C:/tmp')
    @patch('builtins.open', mock_open(read_data="clipboard content"))
    def test_windows_clipboard_handler_read(self, mock_tmp):
        """Test WindowsClipboardHandler read functionality"""
        # Given
        handler = WindowsClipboardHandler(self.runner_mock)
        mock_result = Mock()
        mock_result.returncode = 0
        self.runner_mock.run_command.return_value = mock_result
        ...
        self.runner_mock.run_command.assert_called_once_with(
            [
                ...
                'Get-Clipboard | Set-Content -LiteralPath "C:/tmp\\adb_clipboard_read.txt" -Encoding UTF8'
            ],
    ...
    def test_windows_clipboard_handler_write(self, mock_open, mock_tmp):
        """Test WindowsClipboardHandler write functionality"""
        # Given
        handler = WindowsClipboardHandler(self.runner_mock)
        ...
        self.runner_mock.run_command.assert_called_once_with(
            [
                'powershell', '-NoProfile', '-Command',
                'Set-Clipboard -Value (Get-Content -Raw -Encoding UTF8 -LiteralPath "C:/tmp\\adb_clipboard_write.txt")'
            ],
...
class TestClipboardSyncManager(unittest.TestCase):
    ...
    def test_handle_paste_requests_success(self):
        """Test handling paste requests from multiple devices"""
        # Given
        self.adb_manager.paste_from_device.side_effect = [True, False]

        # When
        result = self.sync_manager._handle_paste_requests(["device1", "device2"])

        # Then
        self.assertTrue(result)
        self.adb_manager.paste_from_device.assert_any_call("device1")
        self.adb_manager.paste_from_device.assert_any_call("device2")
        self.logger.info.assert_called_with("Paste triggered on device1")

    def test_handle_paste_requests_none(self):
        """Test handling when no devices requested paste"""
        # Given
        self.adb_manager.paste_from_device.return_value = False

        # When
        result = self.sync_manager._handle_paste_requests(["device1"])

        # Then
        self.assertFalse(result)
        self.adb_manager.paste_from_device.assert_called_once_with("device1")
